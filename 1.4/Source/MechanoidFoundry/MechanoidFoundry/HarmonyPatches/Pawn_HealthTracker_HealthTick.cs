		...
		private static void Postfix(Pawn_HealthTracker __instance)
		{
				...
				&& __instance.pawn.IsMechanoidHacked() && __instance.pawn.OnMechanoidPad(out bool padIsActive) && padIsActive)
			{
				...
				if (__instance.hediffSet.HasNaturallyHealingInjury())
				{
					...
					{
						toHeal += 4f;
						var building_Bed = __instance.pawn.CurrentBed();
					...
					foreach (var hediff3 in __instance.hediffSet.hediffs)
					{
					...
					var hediffToHeal = (from x in __instance.hediffSet.hediffs.OfType<Hediff_Injury>()
				...
				{
					var hediffs = __instance.hediffSet.hediffs.OfType<Hediff_MissingPart>().ToList();
				...
				{
					var hediff_Injury = (from x in __instance.hediffSet.hediffs.OfType<Hediff_Injury>()
