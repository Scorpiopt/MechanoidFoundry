    ...
    public static class WorkGiver_ConstructDeliverResources_ResetStaticData_Patch
    {
        ...
        public static void CreateMechComponents()
        {
            foreach (var pawn in DefDatabase<PawnKindDef>.AllDefs)
            {
                ...
                {
                    var MF_PlayerMechanoid = DefDatabase<ThinkTreeDef>.GetNamed("MF_PlayerMechanoid");
                    if (!pawn.race.race.thinkTreeMain.thinkRoot.subNodes.Any(x => x is ThinkNode_Subtree subtree && subtree.treeDef == MF_PlayerMechanoid))
                    {
